...
import { LazyLinksEditor } from './LazyLinksEditor';
import { LazyImage } from './LazyImage';

// Lazy load components to reduce initial bundle size
const LazyAppearanceEditor = lazy(() => import('./AppearanceEditor'));
const LazyAnalyticsDashboard = lazy(() => import('./AnalyticsDashboard'));
const LazyShareProfile = lazy(() => import('./ShareProfile'));
const LazyProfilePage = lazy(() => import('./LazyProfilePage'));
    ...
    <div className="h-screen bg-gray-100 flex flex-col md:flex-row overflow-hidden">
      ...
      <div className="w-full md:w-2/5 lg:w-1/3 bg-white p-6 shadow-lg flex flex-col overflow-y-auto">
        ...
        <div className="flex-grow">
          {activeTab === 'links' && (
            <LazyLinksEditor links={profile.links} onUpdateLinks={handleUpdateLinks} />
          )}
          {activeTab === 'appearance' && (
            <Suspense fallback={<div>Carregando editor de aparência...</div>}>
            ...
            <div className="rounded-[2rem] overflow-hidden w-full h-full bg-white">
               <div className="h-full w-full overflow-y-auto">
                 <Suspense fallback={
                   ...
                   <LazyProfilePage profile={profile} onLinkClick={() => {}} />
                 </Suspense>
               </div>
            </div>
          </div>
