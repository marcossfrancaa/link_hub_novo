...
export const LinksEditor: React.FC<LinksEditorProps> = ({ links, onUpdateLinks }) => {
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white rounded-lg p-6 space-y-4 w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => handleAddContent('link')} className="w-full flex items-center p-3 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><LinkLucide className="mr-3 h-5 w-5 text-primary"/> Link</button>
            <button onClick={() => handleAddContent('header')} className="w-full flex items-center p-3 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Type className="mr-3 h-5 w-5 text-primary"/> Header</button>
            <button onClick={() => handleAddContent('youtube')} className="w-full flex items-center p-3 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Youtube className="mr-3 h-5 w-5 text-red-600"/> YouTube Video</button>
            <button onClick={() => handleAddContent('spotify')} className="w-full flex items-center p-3 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors"><Music className="mr-3 h-5 w-5 text-green-500"/> Spotify</button>
          </div>
        </div>
      )}
