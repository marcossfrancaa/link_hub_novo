...
import { LinkIcon, LogoutIcon, PaletteIcon } from './icons';

type AdminTab = 'links' | 'appearance';
...
export const AdminDashboard: React.FC = () => {
  ...
  const handleUpdateLinks = (newLinks: LinkType[]) => {
    updateProfile({ ...profile, links: newLinks });
  };
  ...
  const handleUpdateProfileInfo = (info: { username: string; bio: string; profile_picture_url: string; }) => {
    updateProfile({ ...profile, ...info });
  };

  const handleUpdateSocials = (newSocials: Socials) => {
    updateProfile({ ...profile, socials: newSocials });
  };
